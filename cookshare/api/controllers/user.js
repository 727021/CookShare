...
const { User, Cookbook, Recipe } = require('../models')
...
exports.getAll = async (req, res, next) => {
    try {
        res.send(
            await User.find(
                {},
                `${isAdmin(req) ? 'email ' : ''}username name joined seen admin`
            )
        )
    } catch (err) {
        next(err)
...
exports.postFavorites = async (req, res, next) => {
    ...
    try {
        ...
        if (!req.user.favorites.some(fav => fav.toString() === rid.toString()))
            req.user.favorites.push(rid)
