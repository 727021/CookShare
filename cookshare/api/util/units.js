...
class Unit {
    static find(unit) {
        ...
        return UNITS.find(
            u => u.abbr === unit || u.singular === unit || u.plural === unit
        )
    }
    ...
    static list(measure) {
        ...
        switch (measure) {
            case 'metric':
                return UNITS.filter(
                    u => u.system === 'metric' || u.system === ''
                )
            case 'imperial':
                return UNITS.filter(
                    u => u.system === 'imperial' || u.system === ''
                )
            case 'us':
    ...
    constructor(measure, system, abbr, singular, plural, toBase, fromBase) {
        if (Number(system)) {
            // Unit(abbr|singular|plural|Unit: String|Unit, amount: Number)
            this.amount = system({
                measure,
                system,
                abbr,
                ...
                typeof measure === 'object' && measure !== null
                    ? measure
                    : Unit.find(measure)
