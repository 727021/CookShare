...
const cleanUploads = async () => {
    try {
        ...
        const images = [].concat.apply(
            [],
            (await Recipe.find({}, 'images')).map(r => r.images)
        )
...
const cleanFavorites = async () => {
    try {
        ...
        const recipes = (await Recipe.find()).map(recipe =>
            recipe._id.toString()
        )
        const users = await User.find()
        let deleted = 0
        users.forEach(async user => {
            const newFavorites = user.favorites.filter(fav =>
                recipes.some(r => r === fav)
            )
            if (newFavorites.length !== user.favorites.length) {
                ...
                try {
                    ...
                    logError(
                        err,
                        `clean favorites (${user.username} : ${user._id})`
                    )
                }
            }
        })
